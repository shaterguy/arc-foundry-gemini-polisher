...
import { createInteractiveGeminiProvider } from "../lib/interactive-gemini";
...
test("long interactive provider forces low thinking for rewrite and validation", async () => {
  const oldKey = process.env.GEMINI_API_KEY;
  process.env.GEMINI_API_KEY = "unit-test-key-not-a-secret";
  const input: PolishInput = {
    locked_text: "민재는 복도를 걸었다. 창가에서 잠시 멈췄다. ".repeat(260),
